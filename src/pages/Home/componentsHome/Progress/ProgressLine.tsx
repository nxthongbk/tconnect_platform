...
const ProgressLine = ({props,idx}:{props:Props,idx:number})=>{
    return <Stack spacing={2} width='100%'>
        <Stack direction={'row'} alignItems='center' justifyContent={'space-between'}>
            ...
            <Typography variant='label3' color={'var(--text-primary)'}>{props.title}</Typography>
            </Stack>
            <Typography variant='label3' color={'var(--text-primary)'}>{props.precent}</Typography>
        ...
        <Box component={'div'} className='progress_line-container' >
            ...
            <Box component={'div'} className='affter-precent' left={`calc(${props.precent} - 10px)`}  ></Box>
