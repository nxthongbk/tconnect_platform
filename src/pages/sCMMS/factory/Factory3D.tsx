...
export default function Factory3D({ selectedEquipment, onEquipmentSelect }: Factory3DProps) {
  ...
  const getCameraInfo = (cameraId: string) => {
    ...
    return cameras[cameraId as keyof typeof cameras] || { name: 'Unknown Camera', location: 'Unknown', status: 'Offline' };
  };
  ...
  const getCameraStreamUrl = (cameraId: string) => {
    ...
    return streamUrls[cameraId as keyof typeof streamUrls] || 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=800&h=600&fit=crop';
  };
  ...
  const createSecurityCameras = (scene: Scene, shadowGenerator: ShadowGenerator) => {
    ...
    cameraPositions.forEach(pos => {
      // Camera body
      const cameraBody = MeshBuilder.CreateCylinder(`${pos.id}_body`, { height: 0.8, diameter: 0.6 }, scene);
      cameraBody.position = new Vector3(pos.x, pos.y, pos.z);
      cameraBody.rotation.z = Math.PI / 2;

      // Camera lens
      const cameraLens = MeshBuilder.CreateCylinder(`${pos.id}_lens`, { height: 0.3, diameter: 0.4 }, scene);
      cameraLens.position = new Vector3(pos.x - 0.4, pos.y, pos.z);
      cameraLens.rotation.z = Math.PI / 2;

      // Camera mount
      const cameraMount = MeshBuilder.CreateBox(`${pos.id}_mount`, { width: 0.3, height: 0.3, depth: 0.3 }, scene);
      ...
      cameraBody.actionManager.registerAction(new ExecuteCodeAction(ActionManager.OnPickTrigger, () => {
  ...
  const createProductionLines = (scene: Scene) => {
    // Conveyor belts
    for (let i = 0; i < 3; i++) {
      const conveyor = MeshBuilder.CreateBox(`conveyor${i}`, { width: 25, height: 0.5, depth: 2 }, scene);
  ...
  const createEquipmentMesh = (equipment: Equipment, position: { x: number, z: number }, scene: Scene, shadowGenerator: ShadowGenerator): Mesh => {
    ...
    mesh.position = new Vector3(position.x, mesh.getBoundingInfo().boundingBox.extendSize.y, position.z);
  ...
  const createModernCNCMachine = (id: string, scene: Scene, shadowGenerator: ShadowGenerator): Mesh => {
    ...
    // 2. Cột dọc (Column)
    const column = MeshBuilder.CreateBox(`${id}_column`, { width: 1.5, height: 5, depth: 1.5 }, scene);
    ...
    const spindleHead = MeshBuilder.CreateBox(`${id}_spindleHead`, { width: 2, height: 1.5, depth: 2 }, scene);
    ...
    // 4. Trục cắt (Spindle)
    const spindle = MeshBuilder.CreateCylinder(`${id}_spindle`, { height: 2, diameter: 0.3 }, scene);
    ...
    const workTable = MeshBuilder.CreateBox(`${id}_workTable`, { width: 4, height: 0.3, depth: 3 }, scene);
    ...
    const xRail = MeshBuilder.CreateBox(`${id}_xRail`, { width: 5, height: 0.2, depth: 0.3 }, scene);
    ...
    const yRail = MeshBuilder.CreateBox(`${id}_yRail`, { width: 0.3, height: 0.2, depth: 4 }, scene);
    ...
    const controlCabinet = MeshBuilder.CreateBox(`${id}_controlCabinet`, { width: 1.5, height: 3, depth: 1 }, scene);
    ...
    const controlScreen = MeshBuilder.CreateBox(`${id}_controlScreen`, { width: 1.2, height: 0.8, depth: 0.1 }, scene);
    ...
    const ledLight1 = MeshBuilder.CreateCylinder(`${id}_led1`, { height: 0.1, diameter: 0.3 }, scene);
    ...
    const ledLight2 = MeshBuilder.CreateCylinder(`${id}_led2`, { height: 0.1, diameter: 0.3 }, scene);
    ...
    const coolantTank = MeshBuilder.CreateCylinder(`${id}_coolantTank`, { height: 1.5, diameter: 1 }, scene);
    ...
    // 12. Ống dẫn coolant
    const coolantPipe = MeshBuilder.CreateCylinder(`${id}_coolantPipe`, { height: 3, diameter: 0.1 }, scene);
  ...
  const createModernConveyorBelt = (id: string, scene: Scene, shadowGenerator: ShadowGenerator): Mesh => {
    // Tạo nhóm mesh chính cho băng tải
    const conveyorGroup = new Mesh(`${id}_group`, scene);
    
    // 1. Khung băng tải chính (Main Frame)
    const mainFrame = MeshBuilder.CreateBox(`${id}_mainFrame`, { width: 12, height: 0.8, depth: 3 }, scene);
    ...
    const conveyorBelt = MeshBuilder.CreateBox(`${id}_belt`, { width: 11.5, height: 0.2, depth: 2.5 }, scene);
    ...
    const roller1 = MeshBuilder.CreateCylinder(`${id}_roller1`, { height: 2.8, diameter: 0.6 }, scene);
    roller1.position = new Vector3(-5.5, 0.9, 0);
    roller1.rotation.z = Math.PI / 2;
    roller1.parent = conveyorGroup;
    
    const roller2 = MeshBuilder.CreateCylinder(`${id}_roller2`, { height: 2.8, diameter: 0.6 }, scene);
    ...
    for (let i = -3; i <= 3; i += 2) {
      const supportRoller = MeshBuilder.CreateCylinder(`${id}_supportRoller_${i}`, { height: 2.6, diameter: 0.3 }, scene);
    ...
    legPositions.forEach((pos, index) => {
      const leg = MeshBuilder.CreateCylinder(`${id}_leg_${index}`, { height: 0.8, diameter: 0.2 }, scene);
    ...
    const controlBox = MeshBuilder.CreateBox(`${id}_controlBox`, { width: 1, height: 1.2, depth: 0.6 }, scene);
    ...
    const statusLED = MeshBuilder.CreateCylinder(`${id}_statusLED`, { height: 0.1, diameter: 0.2 }, scene);
    ...
    // 8. Cảm biến (Sensors)
    const sensor1 = MeshBuilder.CreateBox(`${id}_sensor1`, { width: 0.3, height: 0.2, depth: 0.2 }, scene);
    sensor1.position = new Vector3(-3, 1.3, 1.5);
    sensor1.parent = conveyorGroup;
    
    const sensor2 = MeshBuilder.CreateBox(`${id}_sensor2`, { width: 0.3, height: 0.2, depth: 0.2 }, scene);
    ...
    const safetyGuard1 = MeshBuilder.CreateBox(`${id}_safetyGuard1`, { width: 0.1, height: 1, depth: 3 }, scene);
    safetyGuard1.position = new Vector3(-6.2, 1.3, 0);
    safetyGuard1.parent = conveyorGroup;
    
    const safetyGuard2 = MeshBuilder.CreateBox(`${id}_safetyGuard2`, { width: 0.1, height: 1, depth: 3 }, scene);
    ...
    const warningSign = MeshBuilder.CreateBox(`${id}_warningSign`, { width: 0.8, height: 0.6, depth: 0.05 }, scene);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {getCameraInfo(selectedCamera).location}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
